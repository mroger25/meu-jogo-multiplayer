document.addEventListener("DOMContentLoaded", () => {
  ...
  let config = { mundoLargura: 120, mundoAltura: 120, velocidadeJogador: 1 };
  ...
  socket.on("estadoInicial", (estado) => {
    ...
    estado.jogadores.forEach((j) => {
      j.interp_x = j.x; // Posição de desenho
      j.interp_y = j.y; // Posição de desenho
  ...
  socket.on("estadoDelta", (mudancas) => {
    const tipos = {
      jogadorNovo: (m) => {
        console.log(`[CLIENT] Novo jogador apareceu: ${m.id}`);
        // Define a posição inicial e a de interpolação
        m.interp_x = m.x;
        m.interp_y = m.y;
      ...
      jogadorDesconectou: (m) => {
        console.log(`[CLIENT] Jogador desconectou: ${m.id}`);
